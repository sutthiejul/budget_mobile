...
const fs = require("fs");
const app = express();
const TokenManager = require("./token_man");
...
app.post("/get_token/:username", (req, res) => {

//==========Read Secrey Key================================================
const fileName = "./js/token_data.json";
const fileData = fs.readFileSync(fileName, "utf8");
const jsonData = JSON.parse(fileData);

console.log("Secret Key Old :" + jsonData["secret_key"]);
//===========Update Secret Key===============================================
let crypto_random = require("crypto").randomBytes(64).toString("hex");
jsonData["secret_key"] = crypto_random;
...
//===========Get Value Lastest Update===================================================
		const fileData = fs.readFileSync(fileName, "utf8");
		const jsonData = JSON.parse(fileData);
		var SecretKeyNew = jsonData["secret_key"];
		console.log("data : " + SecretKeyNew);
});
//=========================================================================================

  let token = TokenManager.getGenAccessToken({ username: req.params.username });
  //console.log("UserID : " + req.params.username + " ; Token : " + token);
  res.send(token);
});

app.post("/check_token_expire/:token", (req, res) => {
  let tokenCode = req.params.token;
  //console.log(tokenCode);	

  let jwtStatus = TokenManager.checkAuthenToken(tokenCode);

  //console.log("jwtStatus : " + JSON.stringify(jwtStatus)); // object to string

  if (jwtStatus != "" || jwtStatus != false) {
    res.send(JSON.stringify(jwtStatus));
  } else {
...
app.post("/check_token_expire_bearer", (req, res) => {
...
app.get("/getsecretkey", (req, res) => {
  let secret_key = TokenManager.getSecretKey();
  console.log("Secret Key Generate : " + secret_key); // object to string
  res.send("Secret Key Generate : " + secret_key);
});

app.get("/logintoken/:userID", async (req, res) => {
  // check user authen
  try {
    ...
    res.json({ UserID: req.params.userID, Token: accessToken });
  } catch (err) {
    console.log(err);
  }
});
...
io.on("connection", (socket) => {
  ...
  // set userid	admin
  if (users.indexOf("admin") < 0) {
    users.push("admin");
    //console.log("set name : admin");
    console.log("a user connected : " + users.length);
    socket.emit("usrnameSet", { usrname: "admin" });
  }
  ...
  socket.on("clear_user", function () {
    ...
    users.push("admin");
    socket.emit("usrnameSet", { userid: "admin" });
...
// console.log("secret_key :" + jsonData["secret_key"]);
//===========Update Secret Key===============================================
// let crypto_random = require("crypto").randomBytes(64).toString("hex");
// jsonData["secret_key"] = crypto_random;
//jsonData["secret_key"] = "22222222222222222222";
// fs.writeFile(fileName, JSON.stringify(jsonData), function (err) {
//   if (err) {
//     return console.error(err);
//   }
